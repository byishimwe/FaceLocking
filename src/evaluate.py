...
def load_embeddings_for_person(
    ...
    for img_path in imgs:
        ...
        if img is None:
            continue
        if cfg.require_size is not None and not is_aligned_crop(
            img, cfg.require_size
        ):
            continue
...
def sweep_thresholds(
    genuine: np.ndarray, impostor: np.ndarray, cfg: EvalConfig
):
...
def main():
    ...
    for pdir in people_dirs:
        ...
        if len(embs) >= cfg.min_imgs_per_person:
            ...
            print(
                f"Skipping {name}: only {len(embs)} valid aligned crops (need >= {cfg.min_imgs_per_person})"
            )
    ...
    for thr, far, frr in results[::stride]:
        print(
            f"thr: {thr:.2f} | FAR: {far*100:5.2f}% | FRR: {frr*100:5.2f}%"
        )
    ...
    if best is not None:
        ...
        sim_thr = 1.0 - thr
        print(
            f"Equivalent cosine similarity threshold: {sim_thr:.3f} (since sim = 1 - dist)"
        )
    else:
        print(
            f"\nNo threshold in range met target FAR {cfg.target_far*100:.1f}%. "
