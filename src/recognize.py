...
class FaceDBMatcher:
    ...
    def __init__(
        self, db: Dict[str, np.ndarray], dist_thresh: float = 0.34
    ):
        self.db = db
    ...
    def rebuild(self):
        self._names = sorted(list(self.db.keys()))
        if self._names:
            self._mat = np.stack(
                [
                    self.db[n].reshape(-1).astype(np.float32)
    ...
    def match(self, emb: np.ndarray) -> MatchResult:
        if self._mat is None or len(self._names) == 0:
            return MatchResult(
                name=None, distance=1.0, similarity=0.0, accepted=False
            )
...
def main():
    ...
    print(
        "Recognize (multi-face): 'q'=quit, 'r'=reload DB, '+'/'-'=threshold, 'd'=debug overlay"
    )
    t0 = time.time()
    frames = 0
    ...
    while True:
        ...
        for i, f in enumerate(faces):
            cv2.rectangle(
                vis, (f.x1, f.y1), (f.x2, f.y2), (0, 255, 0), 2
            )
        ...
        if key == ord("q"):
            ...
            print(
                f"[recognize] reloaded DB: {len(matcher._names)} identities"
            )
        elif key in (ord("+"), ord("=")):
            matcher.dist_thresh = float(
                min(1.20, matcher.dist_thresh + 0.01)
            )
            print(
            ...
            matcher.dist_thresh = float(
                max(0.05, matcher.dist_thresh - 0.01)
            )
            print(
                f"[recognize] thr(dist)={matcher.dist_thresh:.2f} (sim {1.0-matcher.dist_thresh:.2f})"
            )
            ...
            print(
                f"[recognize] debug overlay: {'ON' if show_debug else 'OFF'}"
            )
